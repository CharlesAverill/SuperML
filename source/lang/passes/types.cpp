...
void unify(Type a, Type b, Subst &S) {
    ...
    if (A->kind != B->kind) {
        throw TypeError("Unification failure: " + stringOfType(A) + " <> " + stringOfType(B));
    }
...
Type infer(const EnvType &env, Term term, Subst &S) {
    if (!term) throw TypeError("infer: null term");
...
Term annotate_term_with_types(const Term &term, Subst &S) {
    ...
    switch (term->kind) {
        ...
        case TermNode::TmLet: {
            ...
            Type ann_t = lt.type ? deref(lt.type, S) : nullptr;
...
Term typecheck(const Term &program) {
    ...
    try {
        ...
        throw TypeError(std::string("Top level does not have type unit: ") + e.what());
    }
